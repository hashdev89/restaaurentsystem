...
const getDefaultCustomizations = (itemId: string): CustomizationGroup[] => {
  // Burgers
...
export function POSSystem() {
  ...
  const filteredItems = useMemo(() => {
    let items = POS_CATEGORIES.flatMap(cat => cat.subItems)
    
    if (selectedCategory) {
      items = POS_CATEGORIES.find(cat => cat.id === selectedCategory)?.subItems || []
    }
  ...
  const quickAddToCart = useCallback((item: POSSubItem) => {
    const category = POS_CATEGORIES.find(cat => 
  ...
  const calculateCustomizationPrice = useCallback((): number => {
    ...
    const customizations = getDefaultCustomizations(selectedItem.id)
    let total = 0
...
<body>
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      try {
        ...
        if (!orderResponse.ok) {
          console.warn('Order creation failed, continuing with local storage only')
        }
      } catch (orderError) {
  ...
  const allItems = useMemo(() => {
    return POS_CATEGORIES.flatMap(cat => 
          ...
          {(selectedCategory || searchQuery) && (
            <div className="flex-1 overflow-y-auto p-4 bg-white">
              {selectedCategory && (
                <div className="mb-4 flex items-center gap-3">
                  ...
                  <h2 className="text-xl font-bold text-gray-900">
                    {POS_CATEGORIES.find(cat => cat.id === selectedCategory)?.name}
                  </h2>
                  <Badge variant="info">
              ...
              {filteredItems.map((item) => {
                const category = POS_CATEGORIES.find((cat) => cat.subItems.some((sub) => sub.id === item.id))
            ...
            <div className="grid grid-cols-4 gap-3">
              {[15, 18, 20, 25].map((percent) => (
                <button
                  ...
                  className={cn(
                    "px-4 py-4 rounded-xl text-base font-bold transition-all active:scale-95 min-h-[56px]",
